                ...
                <form method="GET" class="flex flex-wrap items-end gap-4">
                    <div class="form-field mb-0">
                        ...
                        <select name="month" class="form-control" onchange="this.form.submit()">
                            @for($m = 1; $m <= 12; $m++) <option value="{{ $m }}" {{ $month==$m ? 'selected' : '' }}>
                    ...
                    <div class="form-field mb-0">
                        ...
                        <select name="year" class="form-control" onchange="this.form.submit()">
                            @for($y = 2024; $y <= 2030; $y++) <option value="{{ $y }}" {{ $year==$y ? 'selected' : ''
                                }}>{{ $y }}</option>
                                @endfor
                        </select>
                    </div>
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('user.rekap.export') }}" class="space-y-4">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    ...
                    <label
                        class="inline-flex items-center gap-1.5 cursor-pointer px-3 py-1.5 rounded-lg border transition-colors"
                ...
                <div class="flex flex-wrap items-end gap-4">
                    ...
                    <div x-show="filterType === 'all'" x-cloak>
                        <p class="text-sm text-gray-500 dark:text-gray-400 py-2">
                            <i class="fas fa-info-circle mr-1"></i> Semua data presensi Anda akan didownload
                        </p>
                    </div>
        ...
        <div class="card-body p-0">
            <div class="divide-y divide-gray-100 dark:divide-slate-700">
                ...
                <div class="px-5 py-4 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                    <div class="flex items-start justify-between gap-4">
                            ...
                            <div class="min-w-0 flex-1">
                                <div class="flex items-center gap-2 flex-wrap">
                                    <p class="text-sm font-semibold text-gray-900 dark:text-white">
                                        {{ ($attendance->work_date ?? $attendance->check_in_time)->translatedFormat('l,
                                        d M Y') }}
                                    </p>
                                    @if($attendance->shift)
                                    <span
                                        class="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-600 text-gray-500 dark:text-gray-300 font-medium">
                                ...
                                <div class="flex items-center gap-4 mt-1.5 flex-wrap">
                                    ...
                                    <span
                                        class="inline-flex items-center gap-1 text-xs text-indigo-500 dark:text-indigo-400">
                                        <i class="fas fa-calendar-minus text-[10px]"></i>
                                        Hari Cuti
                                    </span>
                                    @elseif($attendance->status === 'dinas_luar')
                                    <span
                                        class="inline-flex items-center gap-1 text-xs text-blue-500 dark:text-blue-400">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @if($attendance->photo_path && !in_array($attendance->photo_path, ['cuti',
                                        'dinas_luar']))
                                        <button
                                            @click="showModal = true; photoUrl = '{{ asset('storage/' . $attendance->photo_path) }}'; photoTitle = 'Foto Masuk - {{ $attendance->check_in_time->format('d M Y') }}'"
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <button
                                            @click="showModal = true; photoUrl = '{{ asset('storage/' . $attendance->check_out_photo_path) }}'; photoTitle = 'Foto Pulang - {{ $attendance->check_out_time->format('d M Y') }}'"
                        ...
                        <div class="shrink-0">
                            ...
                            @elseif($attendance->status === 'dinas_luar')
                            <span class="badge" style="background-color: rgb(219 234 254); color: rgb(29 78 216);">Dinas
                                Luar</span>
                            @else
                ...
                <div class="px-5 py-12 text-center">
                    <div
                        class="inline-flex h-16 w-16 items-center justify-center rounded-full bg-gray-100 dark:bg-slate-700 mb-3">
                        <i class="fas fa-calendar-times text-2xl text-gray-400 dark:text-gray-500"></i>
                    </div>
                    <p class="font-medium text-gray-900 dark:text-white">Tidak ada data presensi</p>
            ...
            <div @click.away="showModal = false"
                ...
                <div class="flex items-center justify-between p-4 border-b border-gray-100 dark:border-slate-700">
                    ...
                    <button @click="showModal = false"
                        class="h-8 w-8 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-700 flex items-center justify-center transition-colors">
