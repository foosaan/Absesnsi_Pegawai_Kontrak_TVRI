...
<nav class="navbar lg:pl-60">
    <div class="navbar-start">
        {{-- Mobile Menu Toggle --}}
        <div @click.prevent="isAsideMobileExpanded = !isAsideMobileExpanded"
            ...
            <div @click="open = !open" class="navbar-item cursor-pointer">
                <span class="icon icon-lg relative">
                    ...
                    @if(isset($notifications) && $notifications->count() > 0)
                        <span
                            class="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-[10px] text-white">{{ $notifications->count() }}</span>
                    @endif
                </span>
            </div>
            <div x-show="open" @click.outside="open = false" x-transition
                class="dropdown-menu !w-80 !max-h-96 !overflow-y-auto" x-cloak>
                <div class="px-4 py-2.5 flex items-center justify-between border-b dark:border-slate-700">
                    ...
                    @if(isset($notifications) && $notifications->count() > 0)
                        <form method="POST" action="{{ route('notifications.read-all') }}" class="inline">
                            @csrf
                            <button type="submit"
                                class="text-[10px] px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 font-medium hover:bg-blue-200 dark:hover:bg-blue-900/50 transition-colors">
                                Tandai semua dibaca
                            </button>
                        </form>
                    @endif
                ...
                @if(isset($notifications) && $notifications->count() > 0)
                    @foreach($notifications as $notif)
                        <a href="{{ route('notifications.read', $notif->id) }}"
                            class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors border-b border-gray-50 dark:border-slate-700/50 last:border-0">
                            <div
                                class="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-gray-100 dark:bg-slate-700 mt-0.5">
                                <i class="fas {{ $notif->icon }} text-xs {{ $notif->color }}"></i>
                            </div>
                            <div class="min-w-0 flex-1">
                                <p class="text-sm font-medium text-gray-900 dark:text-white leading-tight">{{ $notif->message }}
                                </p>
                                @if($notif->detail)
